    ...
    with closing(get_db()) as session:
        ...
        if user or request:
            ...
            markup = types.InlineKeyboardMarkup(row_width=3).add(*[
                types.InlineKeyboardButton(("Easy"), callback_data=cb_level_choice.new(level=Qualification.Easy, name=message.text)), # type: ignore
                types.InlineKeyboardButton(("Medium"), callback_data=cb_level_choice.new(level=Qualification.Medium, name=message.text)), # type: ignore
                types.InlineKeyboardButton(("Hard"), callback_data=cb_level_choice.new(level=Qualification.Hard, name=message.text)), # type: ignore
                types.InlineKeyboardButton(("Master"), callback_data=cb_level_choice.new(level=Qualification.Master, name=message.text)) # type: ignore
            ])
